...
parser.add_argument('--data_dir', type=str, default='./segment_cell/patches')
...
def feature_extract(datas_path):
    ...
    # del(Model.fc1)
    # del(Model.fc)
    pretrained_dict = torch.load('./SLIC-segmentation/nine_class/nine_class__model.pkl').state_dict()
    ...
    feature_path = os.path.join(datas_path,'patch_feature.pkl')
    patches_feature = torch.load(feature_path)
    for patch_feature in patches_feature:
        patch_name = patch_feature
        patch_path = os.path.join(datas_path,patch_name)
        feature = patches_feature[patch_name]
        coord_x = torch.tensor([int(patch_name.split('_')[0]), int((patch_name.split('_')[1]).split('.')[0]),get_area(cv2.imread(patch_path))]).reshape([1, 3]).cuda()
        feature = torch.cat((feature.detach(), coord_x), dim=1)
        feature_list.append(feature[0].cpu().detach().numpy())

    # datas = os.listdir(datas_path)
    # for data in datas :
    #     if data.split('.')[-1] == 'jpg' and data.split('_')[0].isnumeric():
...
def get_first_config():
    ...
    config.hidden_size = 515 # 512
    config.transformer = ml_collections.ConfigDict()
    config.transformer.mlp_dim = 1024
    config.transformer.num_heads = 5 # 16
...
if __name__ == '__main__':
    ...
    model_path = './model_clam/2-class-515-pretrain_transformer' + str(best_acc) + '.pkl'
